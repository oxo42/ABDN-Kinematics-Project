...
where $\theta_{2u}$ is the shoulder angle for the elbow-up configuration.
for one branch and
\[ \theta_{2d} = \phi - \beta \]

where $\theta_{2d}$  is the shoulder angle for the elbow-down configuration.
...
For $\theta_3$, the forearm angle, we have two cases as above as well: \textbf{elbow-up} and \textbf{elbow-down}. However, the derivation for the inverse kinematic equation for this angle is a bit more complex.
...
\begin{equation}
    \theta_3 = \operatorname{atan2}(\sin \theta_3, \cos \theta_3) \quad \text{(for elbow-down)}
\end{equation}
\begin{equation}
    \theta_3 = \operatorname{atan2}(-\sin \theta_3, \cos \theta_3) \quad \text{(for elbow-up)}
\end{equation}
